...
public class ProductsController {
    ...
    public Product create(@RequestBody Product product) {
        ...
        products.forEach(product1 -> {
            if (product1.equals(product)) unique.set(false);
        });
        if (unique.get()) productRepository.save(product); else throw new ResourceAlreadyExistsException("Product already exists");
        return product;
    }
